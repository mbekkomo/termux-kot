...
-- RFC 5023
local slugtext = _M.RWS / " " + P("%") * (core.HEXDIG * core.HEXDIG / util.read_hex) / string.char + R("\32\126")
...
local comma_sep, comma_sep_trim
do
...
local transfer_parameter = (_M.token - S("qQ") * _M.BWS * P("="))
    * _M.BWS
    * P("=")
    * _M.BWS
    * (_M.token + _M.quoted_string)
local transfer_extension = Cf(
    Ct(_M.token / string.lower) -- case insensitive
        * (_M.OWS * P(";") * _M.OWS * Cg(transfer_parameter)) ^ 0,
    rawset
)
...
local rank = (P("0") * ((P(".") * core.DIGIT ^ -3) / util.safe_tonumber + Cc(0)) + P("1") * ("." * (P("0")) ^ -3) ^ -1)
    * Cc(1)
...
_M.Via = comma_sep_trim(
    Ct(Cg(received_protocol, "protocol") * _M.RWS * Cg(received_by, "by") * (_M.RWS * Cg(_M.comment, "comment")) ^ -1),
    1
)
...
local parameter = _M.token
    / string.lower -- case insensitive
    * P("=")
    * (_M.token + _M.quoted_string)
local media_type = Cg(type, "type")
    * P("/")
    * Cg(subtype, "subtype")
...
local media_range = (P("*/*") + (Cg(type, "type") * P("/*")) + (Cg(type, "type") * P("/") * Cg(subtype, "subtype")))
...
do -- RFC 5988
    ...
    local link_extension = ext_name_star * P("=") * ext_value + parmname * (P("=") * (ptoken + _M.quoted_string)) ^ -1
    -- See https://www.rfc-editor.org/errata_search.php?rfc=5988&eid=3158
    local link_param = link_extension
    local link_value =
        Cf(Ct(P("<") * uri.uri_reference * P(">")) * (_M.OWS * P(";") * _M.OWS * Cg(link_param)) ^ 0, rawset)
...
do -- RFC 6265
    ...
    local extension_av = ((ext_char - S("=") - core.WSP) + core.WSP ^ 1 * #(1 - S("="))) ^ 0 / string.lower * _M.BWS * P("=") * _M.BWS * C(
        ext_char ^ 0
...
do -- RFC 6266
    ...
    local disp_ext_parm = ext_token * _M.OWS * P("=") * _M.OWS * ext_value + _M.token * _M.OWS * P("=") * _M.OWS * value
    local disposition_parm = disp_ext_parm
    _M.Content_Disposition = disposition_type
        * Cf(Ct(true) * (_M.OWS * P(";") * _M.OWS * Cg(disposition_parm)) ^ 0, rawset)
end
...
_M.Depth = P("0") * Cc(0) + P("1") * Cc(1) + case_insensitive("infinity") * Cc(math.huge)
local Simple_ref = uri.absolute_uri + partial_uri
_M.Destination = Simple_ref
local State_token = Coded_URL
local Condition = (case_insensitive("not") * Cc("not")) ^ -1 * _M.OWS * (State_token + P("[") * entity_tag * P("]"))
...
local TimeType = case_insensitive("Second-") * DAVTimeOutVal + case_insensitive("Infinite") * Cc(math.huge)
