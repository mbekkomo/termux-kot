...
local IPvFuture = S("vV")
    * (HEXDIG ^ 1 / util.read_hex)
    * P(".")
    * C((unreserved + _M.sub_delims + P(":")) ^ 1)
    / new_IPvFuture
...
local reg_name = Cs((unreserved / string.lower + _M.pct_encoded / function(s)
    return s:sub(1, 1) == "%" and s or string.lower(s)
end + _M.sub_delims) ^ 1) + Cc(nil)
...
_M.authority = (Cg(_M.userinfo, "userinfo") * P("@")) ^ -1 * Cg(_M.host, "host") * (P(":") * Cg(_M.port, "port")) ^ -1

local hier_part = P("//") * _M.authority * Cg(path_abempty, "path")
    + Cg(path_absolute + path_rootless + path_empty, "path")

_M.absolute_uri = Ct((Cg(_M.scheme, "scheme") * P(":")) * hier_part * (P("?") * Cg(_M.query, "query")) ^ -1)
...
local relative_ref =
    Ct(_M.relative_part * (P("?") * Cg(_M.query, "query")) ^ -1 * (P("#") * Cg(_M.fragment, "fragment")) ^ -1)
